...
function selItemSearchSub[@configid@](field,f,e)
{
    ...
    if(inp.length>=2||f=="sel")
    {
        ...
        if(str!="")
        {
            str = "<div class=\"selItemShow\" style=\"height:10px;padding-bottom:8px;text-align:right\"><span class='searchclose[@configid@]' onclick=\"closeSearchDiv[@configid@](event)\">&Chi;</span></div>"+str;
        }
        dnode.innerHTML=str;
...
const DataListVue[@configid@] = 
{
    ...
    methods: 
    {
        ...
        showImg(imgsrc,imgheight)
        {
            let str = "";
            str  = str + "<img src='./../upload/"+ imgsrc +"' height='" + imgheight + "'>";
            return str;
        },
        listClick(fn,para)
        {
        ...
        listFileName(objfile)
        {
            return objfile;
        },
        listFileDownload(fc)
        {
            let fcobj = JSON.parse(fc);
            let fileName = "";
            let fileGUID="";
            if(Object.prototype.toString.call(fcobj) === '[object Object]')
            {
                fileGUID = fcobj['fileGUID'];
                fileName = fcobj['fileName'];
            }
        ...
        getDatasByFilters(curpage,f) 
        {
            ...
            if(paras==null)
            {
                ...
                if(smode[@configid@]=="")
                {
                    view_filters = document.getElementById("view_prp_filters[@apptype@][@configid@]");
