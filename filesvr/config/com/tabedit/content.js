...
function getBackLocation[@configid@]()
{
    let a = location.href;
    let b = a.split("/");
...
{
    let f = getBackLocation[@configid@]();
...
const DataListVue[@configid@] = 
{
    ...
    methods: 
    {
        ...
        {
            this.isExcel=true;
            gSetSessionStoreNull("dp[@configid@]","[@searchcache@]");
        ...
        goDel() 
        {
            ...
            {
                document.getElementById("content-info-msg[@apptype@][@configid@]").innerHTML="[@infodelsel@]";
            ...
            else
            {
                ...
                if(notids!="")
                {
                    ...
                    document.getElementById("content-info-msg[@apptype@][@configid@]").innerHTML="[@infodelmsg1@]"+ notids +"）[@infodelmsg2@]";
                    document.getElementById("content-info[@apptype@][@configid@]").style.display="block";
                    return;
                }
                this.openWinVue('[@infodelmsg3@]'+ this.selIds.length +'[@infodelmsg4@]',this.delDatas,this.selIds.join(","));
            }
        },
        ...
        showDataItem() 
        {
            ...
            {
                if(item['SYSSTATUS']=='0')
                    item['SYSSTATUS']="[@status1@]";
                if(item['SYSSTATUS']=='1')
                    item['SYSSTATUS']="[@status2@]";
                if(item['SYSSTATUS']=='2')
                    item['SYSSTATUS']="[@status3@]";
            }
        },
        getDataForms(v) 
        {
        ...
        {
            let paras = {};
            let spara = gGetSessionStore("dp[@configid@]","[@searchcache@]");
            if(spara)
            {
                paras = spara['para'];
            ...
            paras['updrights'] = "[@updrights@]";

            var that = this;
            axios.post("./../api/datalist",paras).then(function(res){
            if(res.data&&res.data[0]=="no rights")
            {
                document.getElementById("content-info-msg[@apptype@][@configid@]").innerHTML="[@noright@]";
                document.getElementById("content-info[@apptype@][@configid@]").style.display="block";
                return;
            }
            ...
            gSetSessionStore("dp[@configid@]",paras,that.curpage,"[@searchcache@]");
