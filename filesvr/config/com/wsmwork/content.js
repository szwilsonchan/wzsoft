...
function openApproveWin[@configid@](msg,callback,callbackpara) 
{
    document.getElementById("coverWin[@configid@]").style.display="block";
...
function openWinOK[@configid@](flag) 
{
    window.callback(window.callbackpara,flag); 
}
function closeWin[@configid@]() 
{
    ...
    document.getElementById("WfmWin[@configid@]").style.display="none";
...
const DataListVue[@configid@] = 
{
    ...
    {
        [@btnfunc@]
        showWfmLog(wid,viewcode,itemid)
        {

            let paras = {};
            paras['viewCode']=this.viewcode;
            paras['itemIDs']=itemid;
            paras['wfmworklistid']=wid;
            ...
            document.getElementById("coverWin[@configid@]").style.display="block";
            document.getElementById("WfmWin[@configid@]").style.display="block";
            let dc = document.getElementById("WfmWinMsg[@configid@]");
            dc.innerHTML="";
            ...
            for(let item of wfmdatas)
            {
                ...
                if(item['COMPLETED']=='1')
                {
                    ...
                    if(aform!=null&&aform!="")
                    {
                        aform = "  <a onclick=\"gGoViewLog('" + aform + "','" +item['WFMNODEID']+ "','" +item['WFMWORKID']+ "');void(0)\" >[@capview@]</a>"
                    }
                    else
                    {
                        aform = "";
                    }
                    d.innerHTML="<span><b>[@status3@]("+ item['PSNNAME'] +")</b></span><div><font style='color:grey'>"+ apc + aform + "</font></div><div><font style='color:grey'>"+ apct + "</font></div>"
                    d.className="wfmlog[@configid@]";
                    dc.appendChild(d);
                }
                else
                {
                    d.innerHTML="<span><b>[@status2@]("+ item['PSNNAME'] +")</b></span>"
                    d.className="wfmlog[@configid@]";
                    dc.appendChild(d);
                }
                ...
                if(iindex<wfmdatas.length)
                {
                    ...
                    d.className="wfmlogArrow[@configid@]";
        ...
        goViewLog(sForm,wnodeid,wrkid) 
        {
            ...
            let sf = "form_"+ sForm;
            window.location.href=sf + "?apppsn=1&view=1&wfmworklistid="+ wrkid + "&wfmnodeid="+ wnodeid + "&viewwfmlog=1&backlocation=" + f;
        ...
        goView(sForm,pkey) 
        {
            ...
            sForm = that.goDataForms(forms);
            let sf = "form_"+ sForm;
            window.location.href=sf + "?view=1&approve=1&itemid="+pkey+"&backlocation="+f;
        ...
        goApprove(sForm,pkey,wfmnodeid) 
        {
            let f = pageName[@configid@]();
            f= f +".html";
            ...
            sForm = that.goDataForms(forms);
            let sf = "form_"+ sForm;
            window.location.href=sf + "?wfmworklistid="+pkey+"&wfmnodeid="+ wfmnodeid + "&updwfmform=1&backlocation="+f;

            }).catch(function (err) {
            });

        },
        goApproveView(sForm,pkey,wfmnodeid) 
        {
            let f = pageName[@configid@]();
            f= f +".html";
            ...
            sForm = that.goDataForms(forms);
            let sf = "form_"+ sForm;
            window.location.href=sf + "?view=1&wfmworklistid="+pkey+"&wfmnodeid="+ wfmnodeid + "&viewwfmform=1&backlocation="+f;
        ...
        getDataItemView(itemid) 
        {
            ...
            paras['itemIDs']=itemid;
            var that = this;
            axios.post("./../api/wfmworkget",paras).then(function(res){
        ...
        approveDatas(itemid,flag) 
        {
            ...
            paras['rejectedNode'] = document.getElementById("selRejectNode[@apptype@][@configid@]").value;
            var that = this;
            axios.post("./../api/wfmworkapprove",paras).then(function(res)
            {
                ...
                else
                {
                    ...
                    onchange[@configid@]();
                    closeWin[@configid@]();
                    document.getElementById("content-info-msg[@apptype@][@configid@]").innerHTML="[@appsucc@]";
        ...
        showDataItem() 
        {
            ...
            {
                if(item['WRKCOMPLETED']=='0')
                    item['WRKCOMPLETED']="[@wstatus1@]";
                if(item['WRKCOMPLETED']=='1')
                    item['WRKCOMPLETED']="[@wstatus2@]";
            }
        },
        showDataLbl(v)
        {
        ...
        getDatas () 
        {
            ...
            if(this.mode!="")
                paras['wrkcompleted']=this.mode;

            paras['isdebug'] = "[@isdebug@]";

            var that = this;
            axios.post("./../api/wfmworklist",paras).then(function(res){
        ...
        openUpdWinVue(msg,callback,callbackpara) 
        {
            ...
            let paras = {};
            paras['wfmworklistid']=callbackpara;
            var that = this;
            axios.post("./../api/wfmworkgetlist",paras).then(function(res){
            let wfmdatas=res.data;
            let mRejectNode = {};
            ...
            openApproveWin[@configid@](msg,callback,callbackpara);

            }).catch(function (err) {
            });

        },
        openSearchWinVue(msg,callback,callbackpara) 
        {
    ...
    mounted() 
    {
        ...
        window.gSearchByMode[@configid@]=this.searchByMode;
